...
window.checkoutComplete = function(data) {
    ...
    Paddle.Order.details(checkoutId, function(data) {
        ...
        window.livewire.emit('checkout', { checkout_id: data.checkout.checkout_id });
    });
}
...
window.addEventListener('checkoutCompleteResponse', event => {
...
window.checkoutCancel = function(data){
    ...
    window.livewire.emit('checkoutCancel', { id: subscriptionId });
}

window.addEventListener('checkoutCancelResponse', event => {
    if(parseInt(event.detail.status) == 1){
