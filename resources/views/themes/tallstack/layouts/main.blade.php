...
<body class="flex flex-col min-h-screen @if(Request::is('/')){{ 'bg-white' }}@else{{ 'bg-gray-50' }}@endif @if(config('wave.dev_bar')){{ 'pb-10' }}@endif">
    ...
    <script>
        ...
        @auth
          window.intercomSettings.name = {!! json_encode(auth()->user()->name) !!}; // Full name
          window.intercomSettings.user_id = {!! json_encode(auth()->user()->id) !!}; // a UUID for your user
          window.intercomSettings.email = {!! json_encode(auth()->user()->email) !!}; // the email for your user
